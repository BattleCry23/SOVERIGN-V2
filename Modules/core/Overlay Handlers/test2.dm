...
/**
 * Adds a filter to an overlay
 * @param overlay - The overlay to add filter to (image, obj, or mob)
 * @param filter - The filter to add (single filter or list of filters)
 * @return TRUE if successful, FALSE otherwise
 */
/proc/add_filter(overlay, filter, list/params)
	if(!overlay || !filter) return FALSE
	var/filters_to_add = islist(filter) ? filter : list(filter)
	if(isimage(overlay))
		var/image/img = overlay
		for(var/f in filters_to_add)
			img.filters += f
		return TRUE
	else if(ismob(overlay))
		var/mob/m = overlay
		for(var/f in filters_to_add)
			m.filters += f
		return TRUE
...
/**
 * Removes a filter from an overlay
 * @param overlay - The overlay to remove filter from (image, obj, or mob)
 * @param filter - The filter to remove (single filter or list of filters)
 * @return TRUE if successful, FALSE otherwise
 */
/proc/remove_filter(overlay, filter, list/params)
	if(!overlay || !filter) return FALSE
	var/filters_to_remove = islist(filter) ? filter : list(filter)
	if(isimage(overlay))
		var/image/img = overlay
		for(var/f in filters_to_remove)
			img.filters -= f
		return TRUE
	else if(ismob(overlay))
		var/mob/m = overlay

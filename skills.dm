...
*/
				...
				activate(var/mob/m)
					if(src in m)
						...
						if(!src.last_gain_time || world.time >= src.last_gain_time + src.gain_cd)
							var/sound/SS = sound('basicbeam_chargeoriginal.wav')
							SS.channel = 1
							SS.volume = 30
							SS.repeat = 0
							...
							view(12, m) << SS
							var/sound/S = sound('basicbeam_fire.wav')
		...
		/*Beam
			...
			proc
				...
				activate(var/mob/m)
					...
					var/sound/S = sound('basicbeam_fire.wav'); S.channel = 5; S.volume = 25; S.repeat = 0
			...
			proc
				activate(var/mob/m)
					if(src in m)
						...
						src.cd_max = (initial(src.cd_max)/m.mod_agility)/(1+src.skill_lvl/100)
						m.skill_cooldown(src)
						var/sound/S = sound('Charge_Fire.wav')
						S.channel = 5 // Any number from 1-99
						S.volume = 100
						S.repeat = 0 // Make sure it's not loopin
						while(m)
		...
		Power_Control
			...
			var/obj/aura = null
			info_stats = "Energy Cost: High\n\nMastery: Medium\n\nToggleable"
			var/sound/sfx = sound('Flying Sound FX.ogg')
			var/sound/sfx_start = sound('Power_Control_Stop.wav')
